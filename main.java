public class main {
    public static void main(String[] args) {
        ...
        */
        ...
        /* Когда тебе нужна логика в программирование, ты обращаешься к Булево
         Чтоб найти какое число больше, используется знак сравнения >, которое возвращает истину или ложь */
        
        int firstOne = 22;
        int secondOne = 13;
        System.out.println(firstOne>secondOne); // Выводит true, то есть 22 больше 13
        
        // Упрощенный вариант 
        System.out.println(22>13); // Выводит, что 22 больше 13, true

        int equalNumber = 13;
        System.out.println(equalNumber == 13); // Проверка эквивалентности (совпадения), результат true, так как 13 записано в equalNumber
        
        // Упрощенный вариант 
        System.out.println(10 == 13); // Результат false, так как 10 и 13 не совпадают

        /* Пример, который больше похож на реальную жизнь
         * Нужно понять, может ли человек голосовать (ограничение с 18 лет) */
        int myAge = 20;
        int votingAge = 18;
        System.out.println(myAge >= votingAge); // Выводит true, потому что мой возраст больше, чем ограничение

        /* Модернизация примера выше, чтоб было пользователю понятно, что значит true или false */
        int myAgeM = 20;
        int votingAgeM = 18;   
        if (myAgeM >= votingAgeM) {
            System.out.println("Вы можете голосовать!");
        } else {
            System.out.println("Вы не можете голосовать, вам нет 18"); 
        }

        /* Конструкции if управляют кодом. Если идёт дождь, то нужно взять зонт, а если не идёт, то ничего делать не надо
         * Они решают какой код выполняется, а какой пропускается
         * Всегда для if нужно какое-то условие, чтоб получить результат (true/false) */        
     
        boolean isRaining = true;
        if (isRaining) {
            System.out.println("Возьми зонт!"); // Дождь идёт, зонт берем
        }

        /* Все вариации операторов сравнений  
         * Меньше, чем: a < b
        ...
        /* if использовать тогда, когда условие будет истинное(true) */
        
        if (20 > 18) {
            System.out.println("20 больше, чем 18"); // Так как 20 больше 18, то результат будет true и сообщение выведется
        }
        /* Также, если заменить цифры на переменные содержащие числа, то сравнение будет работать корректно */


        /* else работает, когда условие будет false */
        boolean isRainingNow = false;
        if (isRainingNow) {
            System.out.println("Возьми зонт!"); // Пропускает, так как isRainingNow не true
        } else {
            System.out.println("Дождя нет, необходимости в зонте нет"); // Выводит сообщение, потому что isRainingNow false
        }

        /* else if (elif в других ЯП), задает новое условие, если прошлое было ложно */
        int weather = 2; // 1 = дождь, 2 = солнце на улице, 3 = облачно
        if (weather == 1) {
            System.out.println("Возьми зонт!"); // Это не подходит по условиям
        } else if (weather == 2) {
            System.out.println("Надень очки"); // Так как у нас в переменной погоды указано, что на улице солнце(2), выведет данное сообщение
        } else {
            System.out.println("Просто идём гулять"); // Нашли нужное не доходят до этой проверки
        }

        /* Short hand if...else - включает три операнда
         * Синтаксис выглядит так:
         * variable = (condition) ? expressionTrue :  expressionFalse;
         */
        int time = 20;
        if (time < 18) {
            System.out.println("Хорошего дня"); // Выведет это, но чтоб писать меньше, есть следующий пример
        } else {
            System.out.println("Хорошего вечера");
        }

        int timeEasy = 20;
        String result = (timeEasy < 18) ? "Хорошего дня" : "Хорошего вечера"; // Где прописано сразу 2 условия, зависящее от true и false
        System.out.println(result); // В результате 20 не меньше 18, поэтому выводится вариант для false
